...
import plp.functional1.expression.ArgumentoFuncao;
...
public class DefFuncao {
	...
	protected List<ArgumentoFuncao> argsId;

	protected Expressao exp;

	public DefFuncao(List<ArgumentoFuncao> argsId, Expressao exp) {
		this.argsId = argsId;
		this.exp = exp;
	}

	public List<ArgumentoFuncao> getListaId() {
		return argsId;
	}
	...
	public boolean checaTipo(AmbienteCompilacao ambiente)
		...
		for (ArgumentoFuncao id : argsId) {
			ambiente.map(id.getId(), new TipoPolimorfico());
		}
	...
	public Tipo getTipo(AmbienteCompilacao ambiente)
		...
		for (int i = 0; i < getAridade(); i++) {
			argTipo = ((TipoPolimorfico) ambiente.get(argsId.get(i).getId())).inferir();
			params.add(argTipo);
		}
	...
	public DefFuncao clone() {
		List<ArgumentoFuncao> novaLista = new ArrayList<>(this.argsId.size());
		
		for (ArgumentoFuncao id : this.argsId){
